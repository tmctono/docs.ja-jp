...
Microsoft は、特定の状況を除いて、暗号化解除 (CBC) モードで暗号化されたデータの暗号化解除を、事前に暗号化解除 (CBC) モードで暗号化することができないと考えています。 この略しは、現在認識されている暗号調査に基づいています。

## <a name="introduction"></a>はじめに

埋め込み oracle 攻撃とは、暗号化されたデータに対する攻撃の一種であり、攻撃者はキーを知らなくてもデータの内容を解読することができます。

Oracle は、実行中のアクションが正しいかどうかについての情報を攻撃者に示す "通知" を参照します。 子供とともにボードまたはカードゲームをプレイしてみてください。 優れた機能を備えた顔があるとしたら、それが oracle であると考えています。 相手は、この oracle を使用して、次の移動を適切に計画することができます。
...
- Dampen に対する復号化呼び出しの評価を、タイミングシグナルにします。
  ...
  - 時間の計算は、(ロールオーバー/オーバーフローによる) を使用したり、2つのシステムタイムスタンプ (NTP 調整エラーによる) を引いたりするのではなく、[高解像度のタイムスタンプを取得](/windows/desktop/sysinfo/acquiring-high-resolution-time-stamps)する方法に従って実行する必要があり <xref:System.Environment.TickCount?displayProperty=nameWithType> ます。
  - 時間の計算は、最後に埋め込まれるだけでなく、マネージアプリケーションまたは C++ アプリケーションで発生する可能性のあるすべての例外を含む、復号化操作を含む必要があります。
...
- 暗号化解除呼び出しは、フラグを指定して[Bcryptdecrypt 化](/windows/desktop/api/bcrypt/nf-bcrypt-bcryptdecrypt)を `BCRYPT_BLOCK_PADDING` 行います。
- キーハンドルは、 [BCRYPT_CHAINING_MODE](/windows/desktop/SecCNG/cng-property-identifiers#BCRYPT_CHAINING_MODE)がに設定された[bcryptsetproperty](/windows/desktop/api/bcrypt/nf-bcrypt-bcryptsetproperty)を呼び出すことによって初期化されてい `BCRYPT_CHAIN_MODE_CBC` ます。
  - は既定値であるため `BCRYPT_CHAIN_MODE_CBC` 、影響を受けるコードにはの値が割り当てられていない可能性があり `BCRYPT_CHAINING_MODE` ます。

以前の Windows 暗号化 API に対してビルドされたプログラムの場合:

- 復号化の呼び出しは、を使用して暗号[解読](/windows/desktop/api/wincrypt/nf-wincrypt-cryptdecrypt)を `Final=TRUE` 行います。
- キーハンドルは、 [KP_MODE](/windows/desktop/api/wincrypt/nf-wincrypt-cryptgetkeyparam)をに設定して[CryptSetKeyParam](/windows/desktop/api/wincrypt/nf-wincrypt-cryptsetkeyparam)を呼び出すことによって初期化されてい `CRYPT_MODE_CBC` ます。
  - は既定値であるため `CRYPT_MODE_CBC` 、影響を受けるコードにはの値が割り当てられていない可能性があり `KP_MODE` ます。

## <a name="finding-vulnerable-code---managed-applications"></a>脆弱なコード管理アプリケーションの検索

- 復号化呼び出しは、 <xref:System.Security.Cryptography.SymmetricAlgorithm.CreateDecryptor> <xref:System.Security.Cryptography.SymmetricAlgorithm.CreateDecryptor(System.Byte[],System.Byte[])> のメソッドまたはメソッドに対して <xref:System.Security.Cryptography.SymmetricAlgorithm?displayProperty=nameWithType> 行います。
...
- <xref:System.Security.Cryptography.SymmetricAlgorithm.Padding?displayProperty=nameWithType>プロパティが <xref:System.Security.Cryptography.PaddingMode.PKCS7?displayProperty=nameWithType> 、、またはに設定されました <xref:System.Security.Cryptography.PaddingMode.ANSIX923?displayProperty=nameWithType> <xref:System.Security.Cryptography.PaddingMode.ISO10126?displayProperty=nameWithType> 。
  - <xref:System.Security.Cryptography.PaddingMode.PKCS7?displayProperty=nameWithType>は既定値であるため、影響を受けるコードにはプロパティが割り当てられていない可能性があり <xref:System.Security.Cryptography.SymmetricAlgorithm.Padding?displayProperty=nameWithType> ます。
- <xref:System.Security.Cryptography.SymmetricAlgorithm.Mode?displayProperty=nameWithType>プロパティがに設定されました。<xref:System.Security.Cryptography.CipherMode.CBC?displayProperty=nameWithType>
  - <xref:System.Security.Cryptography.CipherMode.CBC?displayProperty=nameWithType>は既定値であるため、影響を受けるコードにはプロパティが割り当てられていない可能性があり <xref:System.Security.Cryptography.SymmetricAlgorithm.Mode?displayProperty=nameWithType> ます。

## <a name="finding-vulnerable-code---cryptographic-message-syntax"></a>脆弱性のあるコードの検出-暗号化メッセージの構文

暗号化されたコンテンツが、AES の CBC モード (2.16.840.1.101.3.4.1.2、2.16.840.1.101.3.4.1.22、2.16.840.1.101.3.4.1.42)、DES (1.3.14.3.2.7)、3DES (1.2.840.113549.3.7)、または RC2 (1.2.840.113549.3.2) を使用する、認証されていない CMS EnvelopedData メッセージと、CBC モードで他のブロック暗号アルゴリズムを使用するメッセージ。

ストリーム暗号はこの特定の脆弱性の影響を受けませんが、Microsoft では、ContentEncryptionAlgorithm 値を検査することによって常にデータを認証することをお勧めします。

マネージアプリケーションの場合、CMS EnvelopedData blob はに渡される任意の値として検出でき <xref:System.Security.Cryptography.Pkcs.EnvelopedCms.Decode(System.Byte[])?displayProperty=fullName> ます。

ネイティブアプリケーションの場合、cms EnvelopedData blob は、 [Cryptmsgupdate](/windows/desktop/api/wincrypt/nf-wincrypt-cryptmsgupdate)を介して cms ハンドルに提供される任意の値として検出できます。この場合、その結果[CMSG_TYPE_PARAM](/windows/desktop/api/wincrypt/nf-wincrypt-cryptmsggetparam) `CMSG_ENVELOPED` となります。また、cms ハンドルは後で、 `CMSG_CTRL_DECRYPT` [cryptmsgupdate](/windows/desktop/api/wincrypt/nf-wincrypt-cryptmsgcontrol)経由で命令を送信します。
...
`cipher_algorithm_id`と `hmac_algorithm_id` アルゴリズム識別子は、これらのアルゴリズムのアプリケーションローカルの (標準ではない) 表現です。 これらの識別子は、ベア連結されたバイトストリームとしてではなく、既存のメッセージングプロトコルの他の部分でも意味があります。
