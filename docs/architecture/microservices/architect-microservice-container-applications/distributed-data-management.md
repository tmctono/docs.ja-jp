...
**クエリ/読み取りテーブルを使う CQRS。** 複数のマイクロサービスからデータを集計するもう 1 つの解決策は、[具体化されたビュー パターン](/azure/architecture/patterns/materialized-view)です。 この方法では、複数のマイクロサービスによって所有されているデータを含む読み取り専用テーブルを、事前に (実際のクエリが行われる前に非正規化データを準備します) 生成します。 このテーブルはクライアント アプリのニーズに合った形式にします。
