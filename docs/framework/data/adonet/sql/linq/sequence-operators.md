...
ms.locfileid: "91173446"
---
# <a name="sequence-operators"></a><span data-ttu-id="d9ad2-102">シーケンス演算子</span><span class="sxs-lookup"><span data-stu-id="d9ad2-102">Sequence Operators</span></span>

<span data-ttu-id="d9ad2-103">一般的には、[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] では、次のいずれかの性質を持つシーケンス演算子はサポートされません。</span><span class="sxs-lookup"><span data-stu-id="d9ad2-103">Generally speaking, [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] does not support sequence operators that have one or more of the following qualities:</span></span>  
  
- <span data-ttu-id="d9ad2-104">ラムダとインデックス パラメーターを使用する。</span><span class="sxs-lookup"><span data-stu-id="d9ad2-104">Take a lambda with an index parameter.</span></span>  
  
- <span data-ttu-id="d9ad2-105">シーケンス行のプロパティに依存する (<xref:System.Linq.Queryable.TakeWhile%2A> など)。</span><span class="sxs-lookup"><span data-stu-id="d9ad2-105">Rely on the properties of sequential rows, such as <xref:System.Linq.Queryable.TakeWhile%2A>.</span></span>  
  
- <span data-ttu-id="d9ad2-106">任意の CLR 実装に依存する (<xref:System.Collections.Generic.IComparer%601> など)。</span><span class="sxs-lookup"><span data-stu-id="d9ad2-106">Rely on an arbitrary CLR implementation, such as <xref:System.Collections.Generic.IComparer%601>.</span></span>  
  
|<span data-ttu-id="d9ad2-107">サポートされない例</span><span class="sxs-lookup"><span data-stu-id="d9ad2-107">Examples of Unsupported</span></span>|  
|-----------------------------|  
...
## <a name="differences-from-net"></a><span data-ttu-id="d9ad2-108">.NET との相違</span><span class="sxs-lookup"><span data-stu-id="d9ad2-108">Differences from .NET</span></span>  

 <span data-ttu-id="d9ad2-109">サポートされるすべてのシーケンス演算子は、`Average` を除いて、共通言語ランタイム (CLR) で正常に動作します。</span><span class="sxs-lookup"><span data-stu-id="d9ad2-109">All supported sequence operators work as expected in the common language runtime (CLR) except for `Average`.</span></span> <span data-ttu-id="d9ad2-110">`Average` は、平均化される型と同じ型の値を返しますが、CLR での `Average` は、常に <xref:System.Double> または <xref:System.Decimal> を返します。</span><span class="sxs-lookup"><span data-stu-id="d9ad2-110">`Average` returns a value of the same type as the type being averaged, whereas in the CLR `Average` always returns either a <xref:System.Double> or a <xref:System.Decimal>.</span></span> <span data-ttu-id="d9ad2-111">ソース引数が明示的に double / decimal にキャストされている場合、または、セレクターが double / decimal にキャストする場合、作成される SQL でもこのような変換が行われるため、結果に問題はありません。</span><span class="sxs-lookup"><span data-stu-id="d9ad2-111">If the source argument is explicitly cast to double / decimal or the selector casts to double / decimal, the resulting SQL will also have such a conversion and the result will be as expected.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="d9ad2-112">関連項目</span><span class="sxs-lookup"><span data-stu-id="d9ad2-112">See also</span></span>

- [<span data-ttu-id="d9ad2-113">データ型と関数</span><span class="sxs-lookup"><span data-stu-id="d9ad2-113">Data Types and Functions</span></span>](data-types-and-functions.md)
