...
|`For`|`seq<'T> * ('T -> M<'U>) -> M<'U>`<br /><br />`seq<'T> * ('T -> M<'U>) -> seq<M<'U>>`|コンピュテーション式の式に対して呼び出され `for...do` ます。|
...
次の例は、既存のクラスの拡張を示して `Microsoft.FSharp.Linq.QueryBuilder` います。
