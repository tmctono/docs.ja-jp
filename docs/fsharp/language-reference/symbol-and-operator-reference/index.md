...
次の表では、F# 言語で使用されるシンボルについて説明し、シンボルの使用方法の一部に関する簡単な説明と、より詳しい情報へのリンクを示します。 シンボルは、ASCII 文字セットの順序に従って並べられています。
...
|`!=`||<ul><li>F# では使用されません。 非等値演算には `<>` を使用します。<br /></li></ul>|
...
|`$`||<ul><li>コンパイラで生成された特定の変数名と関数名に、内部的に使用されます。<br /></li></ul>|
...
|<code>&#96;&#96;...&#96;&#96;</code>||<ul><li>言語のキーワードなど、区切らなければ有効な識別子にならない識別子を区切ります。<br /></li></ul>|
...
|`:?`|[match 式](../match-expressions.md)|<ul><li>値が指定した型と一致する場合 (サブタイプの場合を含む) は `true` を返します。それ以外の場合は、`false` を返します (型テスト演算子)。<br /></li></ul>|
...
|`==`||<ul><li>F# では使用されません。 等値演算には `=` を使用します。<br /></li></ul>|
...
|`>=`|[算術演算子](arithmetic-operators.md)|<ul><li>左辺が右辺以上である場合は `true` を返します。それ以外の場合は `false` を返します。<br /></li></ul>|
...
|`? ... <- ...`||<ul><li>動的プロパティを設定するための演算子として使用されます。 独自の実装を提供する必要があります。<br /></li></ul>|
